...
from det3d.ops.bev_pool_v2.bev_pool import bev_pool_v2
from mmdet.models.backbones.resnet import BasicBlock
from mmdet3d.models.builder import NECKS


@NECKS.register_module()
class ViewTransformerLiftSplatShootv2(BaseModule):
    ...
    def __init__(self,
        ...
        self.depthnet = nn.Conv2d(
            numC_input, self.D + self.numC_Trans, kernel_size=1, padding=0)
        self.accelerate = accelerate
        self.initial_flag = True

    def update_grid_varaible(self):
        self.grid_config['x'] = self.grid_config['xbound']
        self.grid_config['y'] = self.grid_config['ybound']
        self.grid_config['z'] = self.grid_config['zbound']
        self.grid_config['depth'] = self.grid_config['dbound']
    ...
    def create_grid_infos(self, x, y, z, **kwargs):
        ...
        self.grid_size = torch.Tensor([(cfg[1] - cfg[0]) / cfg[2]
                                       for cfg in [x, y, z]])
        self.dx = torch.Tensor([row[2] for row in [x, y, z]])
        self.bx = torch.Tensor([row[0] + row[2]/2.0 for row in [x, y, z]])
        self.nx = torch.Tensor([(row[1] - row[0]) / row[2] for row in [x, y, z]])
    ...
    def get_lidar_coor(self, rots, trans, cam2imgs, post_rots, post_trans):
        ...
        """
        B, N, _ = trans.shape
        is_half = rots.dtype == torch.float16
        # post-transformation
        # B x N x D x H x W x 3
        points = self.frustum.to(rots.float()) - post_trans.float().view(B, N, 1, 1, 1, 3)
        points = torch.inverse(post_rots.float()).view(B, N, 1, 1, 1, 3, 3)\
        ...
        points += trans.float().view(B, N, 1, 1, 1, 3)
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        # Lift-Splat
        if self.accelerate:
            feat = tran_feat.view(B, N, self.numC_Trans, H, W)
            feat = feat.permute(0, 1, 3, 4, 2)
            ...
            bev_feat = self.voxel_pooling_v2(
                ...
                tran_feat.view(B, N, self.numC_Trans, H, W))
...
@NECKS.register_module()
class ViewTransformerLiftSplatShootDepthv2(ViewTransformerLiftSplatShootv2):
    def __init__(self,
                 loss_depth_weight=3.0, 
                 depthnet_cfg=dict(),
                 **kwargs):
    
        super(ViewTransformerLiftSplatShootDepthv2, self).__init__(**kwargs)
        self.loss_depth_weight = loss_depth_weight
        self.depthnet = DepthNet(self.numC_input,
                                  self.numC_input,
                                  self.numC_Trans,
                                  self.D,
                                  **depthnet_cfg)
    ...
    def forward(self, input):
        (x, rots, trans, intrins,\
            post_rots, post_trans, depth_gt) = input[:8]
