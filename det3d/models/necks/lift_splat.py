...
class ViewTransformerLiftSplatShoot(BaseModule):
    ...
    def get_geometry(self, rots, trans, intrins, post_rots, post_trans, offset=None):
        ...
        if post_rots.dtype == torch.float16:
            post_rots = post_rots.float()
        ...
        # new_points = self.frustum.data.clone()
        # new_points = torch.cat([new_points,
        #                         torch.ones_like(new_points)[...,0:1]], dim=-1)
        # transformed_points = points.clone()
        # transformed_points = torch.cat([transformed_points,
        #                                 torch.ones_like(transformed_points)[..., 0:1]], dim=-1)
        # cam_matrix = torch.eye(4, 4).to(new_points.device)
        # cam_matrix = cam_matrix[None, None, :,:].repeat(B, N, 1, 1)

        # post_transform = cam_matrix.clone()
        # intrins_transform = cam_matrix.clone()
        # transform = cam_matrix.clone()
        ...
        # recover_transform = post_transform.matmul(intrins_transform).matmul(torch.inverse(transform))
        # import pdb; pdb.set_trace()

        # stage3_transform = torch.inverse(transform)
        # recover_stage3_points = stage3_transform.view(B, N, 1, 1, 1, 4, 4).matmul(transformed_points.unsqueeze(-1))
        # recover_stage3_points = recover_stage3_points.squeeze(-1)
        # import pdb; pdb.set_trace()


        # stage2_tranform = intrins_transform.matmul(torch.inverse(transform))
        # recover_stage2_points = stage2_tranform.view(B, N, 1, 1, 1, 4, 4).matmul(transformed_points.unsqueeze(-1))
        # recover_stage2_points = recover_stage2_points.squeeze(-1)
        # recover_stage2_points[...,:2] = recover_stage2_points[...,:2] / recover_stage2_points[...,2:3]

        # recover_stage0_points = post_transform.view(B, N,  1, 1, 1, 4, 4).matmul(recover_stage2_points.unsqueeze(-1))
        # recover_stage0_points = recover_stage0_points.squeeze(-1)
        # import pdb; pdb.set_trace()

        # origin_points = recover_transform.view(B, N, 1, 1, 1, 4, 4).matmul(transformed_points.unsqueeze(-1))
        # origin_points = origin_points.squeeze(-1)
        # origin_points[...,:2] = origin_points[..., :2] / origin_points[..., 2:3]
        # # transform =
        # # check how to do the inversei
...
@NECKS.register_module()
class ViewTransformerLiftSplatShootDepth(ViewTransformerLiftSplatShoot):
    def __init__(self,
                 extra_depth_net,
                 loss_depth_weight,
                 se_config=dict(),
        ...
        super(ViewTransformerLiftSplatShootDepth, self).__init__(**kwargs)
