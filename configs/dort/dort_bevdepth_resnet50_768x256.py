...
model = dict(
    ...
    neck_bev=dict(
        ...
        numC_input=numC_Trans*9,
        num_channels=[numC_Trans*9,
                        numC_Trans*18,
                        numC_Trans*36],
        neck=dict(
            type='FPN_LSS',
            in_channels=numC_Trans*36+numC_Trans*9,
            out_channels=256)),
    pre_process = dict(type='ResNetForBEVDet',numC_input=numC_Trans,
                      num_layer=[2,], num_channels=[64,], stride=[1,],
                      backbone_output_ids=[0,]),
    view_transform=dict(type='ViewTransformerLiftSplatShootDepth',
                        grid_config=grid_config,
                        data_config=data_config,
                        numC_Trans=numC_Trans,
                        loss_depth_weight=100.0,
                        dcn_config=dict(bias=False),
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            box_type_3d='LiDAR',
            img_info_prototype='bevdet_sequentialv2',
            speed_mode=None,
            prev_only=True,
            fix_direction=True)),
    val=dict(
