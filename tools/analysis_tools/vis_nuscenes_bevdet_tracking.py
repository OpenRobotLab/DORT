...
def check_point_in_img_idx(x, y, height, width):
    if x>=0 and y>=0 and x<width and x<height:
        return True
    else:
        return False
def random_color(seed=0):
    ...
    color = [int(i*255) for i in list(color)]
    color = tuple(color)
    return color
...
def parse_args():
    ...
    parser.add_argument('--info-path',
                        default="nuscenes_mini_infos_val.pkl",
                        help='Path to the pkl path in mmdet3d format')
...
def sample_color(tracking_color_dict, tracking_ids):
    colors = []
    for tracking_id in tracking_ids:
        if tracking_id in tracking_color_dict:
            colors.append(tracking_color_dict[tracking_id])
        else:
            color = random_color(int(tracking_id))
            tracking_color_dict[tracking_id] = color
            colors.append(color)
    return tracking_color_dict, colors
...
def main():
    ...
    info_path = osp.join(args.root_path, args.info_path)
    ...
    # TODO check how to filter class
    #
    tracking_color_dict = {}
    ...
    for cnt, infos in enumerate(dataset['infos'][:min(args.vis_frames,
        ...
        tracking_ids = [box['tracking_id'] for box in res['results'][infos['token']]]

        tracking_color_dict, colors = sample_color(tracking_color_dict, tracking_ids)
        ...
        scores = [pred_res[rid]['tracking_score'] for rid in
                  range(len(pred_res))]
        if args.draw_gt:
            box = infos['gt_boxes']
            # tracking_id = info
            names = infos['gt_names']
            valid_mask = [name in tracking_class for name in names]
            valid_mask = np.array(valid_mask).astype(np.bool)
            box = box[valid_mask]
            if 'anns_token' in infos:
                anns_token = infos['anns_token']
                anns_token = anns_token[valid_mask]
            else:
                anns_token = None
            if valid_mask.sum() > 0:    
                corners_lidar_gt = \
        ...
        for view in views:
            ...
            valid_depth = copy.deepcopy(valid)
            valid = np.logical_and(valid,
            ...
            for aid in range(valid.shape[0]):
                ...
                # # print_tracking id
                # txt_coordinate_x, txt_coordinate_y = corners_img[aid, index[0]]
                # txt_coordinate_y-=2
                # txt_coordinate_x+=2
                # if valid_depth[aid] and check_point_in_img_idx(
                #         txt_coordinate_x, txt_coordinate_y, img.shape[0], img.shape[1]):
                #     cv2.putText(img,
                #                 str(tracking_ids[aid]),
                #                 (txt_coordinate_x, txt_coordinate_y),
        ...
        for rid in sort_ids:
            ...
            cv2.line(canvas, center_canvas[rid], head_canvas[rid],
                     ...
                     1, lineType=8)
            # print tracking id
            txt_coordinate_x, txt_coordinate_y = bottom_corners_bev[rid, index[0]]
            txt_coordinate_y -= 2
            # if valid_depth[aid] and check_point_in_img_idx(
            #         txt_coordinate_x, txt_coordinate_y, img.shape[0], img.shape[1]):
            #     # cv2.putText(img,
